...
public class DrawInformationPreNucleiFork extends RecursiveAction {    
    ...
    private final CytoplasmStaining se;
    private final TMAspot ts;
    private final List<TMALabel> tps;
    private final List<LocalizedROI> lROIs;
     ...
     */
    public DrawInformationPreNucleiFork(TMARKERPluginManager tpm, CytoplasmStaining se, TMAspot ts, List<TMALabel> tps, List<LocalizedROI> lROIs, BufferedImage I, Graphics g, double z, int x_min, int y_min, int x_max, int y_max, int mStart, int mLength, boolean doFork, int[] progress_value) {
        this.tpm = tpm;
        this.se = se;
        this.ts = ts;
        this.tps = tps;
    ...
    protected void compute() {
        ...
        for (int i=0; i<n_proc; i++) {
            ...
            if (split_adj>0) {
                fjt.add(new DrawInformationPreNucleiFork(tpm, se, ts, tps, lROIs, I, g, z, x_min, y_min, x_max, y_max, mStart + i*split, split_adj, false, progress_container));
            }
        }
        invokeAll(fjt);
    }
    ...
    protected void computeDirectly() {
        for (int i=mStart; i<mStart+mLength; i++) {
            if (!interrupt) {
                CytoplasmStaining.drawInformationPreNucleus(se, ts, tps.get(i), lROIs.get(i), I, g, z, x_min, y_min, x_max, y_max);
