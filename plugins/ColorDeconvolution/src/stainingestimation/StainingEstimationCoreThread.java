...
public class StainingEstimationCoreThread extends Thread {    
     ...
     */
    public StainingEstimationCoreThread (TMARKERPluginManager tpm, StainingEstimation se, TMAspot aTMAspot, int radius, double blur, int tolerance, int TMblur_hema, int TMblur_dab, int t_hema, int t_dab, boolean hide_legend, String myStain, boolean substractChannels, boolean useThresholdMap, boolean respectAreas, List<TMApoint> brown_spots_total, List<Point> offsets, List<Point> sizes, int maxsize) {
        this.tpm = tpm;
        this.se = se;
    ...
    public void run() {
        ...
        try{ 
            ...
            fb = new StainingEstimationCoreFork(tpm, se, aTMAspot, radius, blur, tolerance, TMblur_hema, TMblur_dab, t_hema, t_dab, hide_legend, myStain, substractChannels, useThresholdMap, respectAreas, brown_spots_total, offsets, sizes, maxsize, 0, offsets.size(), tpm.useParallelProgramming(), progress_container);
