...
public class Normalize_ implements PlugInFilter {
	...
	public void run(ImageProcessor ip) {
		...
		case ImagePlus.GRAY16:
			...
			processGray32Unsigned(dScale, ip, impBright, impDark, iLen);
			break;
	...
	void processGray16Unsigned(double dScale, ImageProcessor ip, ImagePlus impBright, 
        ...
        /**
	 * Normalize a 32-bit unsigned image NOT TESTED
	 */
	void processGray32Unsigned(double dScale, ImageProcessor ip, ImagePlus impBright, 
							 ImagePlus impDark, int iLen) {
		if ((impBright != null) && (impDark != null)) {
			int[] asImage = (int[])ip.getPixels();
			int[] asBright = (int[])impBright.getProcessor().getPixels();
			int[] asDark = (int[])impDark.getProcessor().getPixels();
			for (int i=0; i<iLen; i++) {
				asImage[i] = (int)(dScale * ((asImage[i]&0xffffffff) - (asDark[i]&0xffffffff)) / ((asBright[i]&0xffffffff) - (asDark[i]&0xffffffff)));
			}
		} else if (impBright != null) {
			int[] asImage = (int[])ip.getPixels();
			int[] asBright = (int[])impBright.getProcessor().getPixels();
			for (int i=0; i<iLen; i++) {
				asImage[i] = (int)(dScale * (asImage[i]&0xffffffff) / (asBright[i]&0xffffffff));
			}
		} else if (impDark != null) {
			int[] asImage = (int[])ip.getPixels();
			int[] asDark = (int[])impDark.getProcessor().getPixels();
			for (int i=0; i<iLen; i++) {
				asImage[i] = (int)((asImage[i]&0xffffffff) - (asDark[i]&0xffffffff));
			}
		}
	}
